...
#
## Combine with county suicide data
#
df <- merge(df, raw_s_df, by.x = "facility_county", by.y = "CountyState")

# Keep only columns related to model 
## Only select columns that actually exist in the raw data
selected_cols[!selected_cols %in% c("facility_county", "CountyState")]
...
# Wrangle -- convert all character strings to Factors
Cols2Factors <- names(select(df, where(~ !is.numeric(.))))
