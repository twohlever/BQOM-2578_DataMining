...
df <- df %>% mutate(across(Cols2Numeric, as.numeric))
df$target <- as.numeric(as.character(df$target))

# Wrangle -- convert character strings to Factors
Cols2Factors <- c( 
    "type_of_organization",
    "type_of_services",
    "children_hospital",
...
df <- df %>% mutate(across(Cols2Factors, as.factor))
df <- df %>%
...
cor_mat <- cor(df)
cols_above_threshold <- which(apply(cor_mat, 2, function(col) any(abs(col) > 0.6)))
...
m_sex<-lm(Obs_Count ~ sex_id, data=df)
summary(m_sex)

m_sex_age <-lm(Obs_Count ~ sex_id + age_id, data=df)
summary(m_sex_age)
