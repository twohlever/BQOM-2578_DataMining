---
title: "Logistic Regression with Credit Data"
format: docx
editor: visual
---
...
#
# Go to the Global Environment and click on Coeftable for another view
#
```
...
ConfMatrix<-function(actual_value,predicted_prob,cutoff){   
...
```{r}
ConfMatrix(loans$Default,loans$PredProbs1,0.25)

#ConfMatrix(loans$Default,loans$PredProbs1,0.15)
#ConfMatrix(loans$Default,loans$PredProbs1,0.05)
```
...
```{r}
MyConfMatrixValues<-function(actual_value,predicted_prob,cutoff){
...
```{r}
MyConfMatrixValues(loans$Default,loans$PredProbs1,0.25)
```
...
MyConfMatrixValues(loans$Default,loans$PredProbs1,0.4)
```
...
```{r}
MyConfMatrixValues(loans$Default,loans$PredProbs2,cutoff=0.55)
```
...
MyConfMatrixValues(test.df$Default,test.df$PredProbs2b,cutoff=0.55)
```
